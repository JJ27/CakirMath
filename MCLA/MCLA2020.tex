...
For a square matrix $A$, we want to find if there are any nonzero vectors $\vv{x}$ such that $A\vv{x} = \lambda \vv{x}$, where $\lambda$ is a scalar. This is called an \textit{eigenvalue problem}, where $\lambda$ is the \textit{eigenvalue} and $\vv{x}$ is the \textit{eigenvector}.

...
The connection between surface integrals and surface area are similar to line integrals and arc length. The integral is just adding \textit{something} over the whole space. So the form for the surface integral would be:
\begin{gather*}
...
To orient a surface(like we did with a curve) we need to make sure it's possible. For example, the \textbf{Mobius Strip} only has one side and so it's not orientable. But for curves that are, the would be two possible normal vectors from each point, one that goes into the surface and one that goes out, such that $\vv{n_1} = -\vv{n_2}$.
...
\begin{gather*}
    \iint_S\vv{F}\vdot d\vv{S} = \iint_S \vv{F}\vdot\vv{n}\,dS = \iint_D \vl P,Q,R\vr \vdot \frac{\vl -g_x, -g_y, 1}{\sqrt{1 + (g_x)^2 + (g_y)^2}} \sqrt{1 + (g_x)^2 + (g_y)^2}\,dA\\
...
Using the same idea:
\begin{gather*}
    \iint_S \vv{F}\vdot \vv{S} = \iint_S \vv{F}\vdot\frac{\vv{r_u} \times \vv{r_v}}{|{\vv{r_u} \times \vv{r_v}|}}\,dS = \iint_D \bigg[\vv{F}(\vv{r}(u,v))\vdot\frac{\vv{r_u} \times \vv{r_v}}{|{\vv{r_u} \times \vv{r_v}|}}\bigg]|\vv{r_u} \times \vv{r_v}|\,dA\\
...
\end{gather*}
This is just found by integrating both sides around the surface to add up the total heat flow on the surface per unit time.a
\part{Linear Algebra}

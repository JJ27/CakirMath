...
We know from the last chapter that not all square matrices are diagonalizable, but this changes if we only consider \textit{symmetric matrices}. All eigenvalues of a real symmetric matrix are going to be real and diagonalizable. If you take the old diagonalization process, we found a matrix $P$ such that $P^{-1}AP = D$, where $P$ is the matrix of eigenvectors. However, if we normalize those vectors we find that those vectors are actually orthonormal! A square matrix is \textbf{orthogonally diagonalizable} if there is an orthogonal matrix $Q$ that satisfies $Q^{-1} AQ = Q^T AQ = D$.

If $A$ satisfies the above definition, then it must be symmetric.
\begin{proof}
Since $Q^{-1} = Q^T$, $Q^TQ = QQ^T = I$:
\begin{gather*}
    QDQ^T = Q(Q^TAQ)Q^T = IAI = A\\
    A^T = (QDQ^T)^T = (Q^T)^TD^TQ^T = QDQ^T = A
\end{gather*}
So $A$ is symmetric since $A = A^T$.
\end{proof}
if $A$ is symmetric and real, then its eigenvalues are also real.
We need to reintroduce the notion of a conjugate for a complex number. $z = a + bi$ then $\bar{z} = a - bi$. We can extend this to matrices by saying that $\bar{A} = [\overline{a_{ij}}]$.
\begin{proof}
We know that $\overline{A\vv{v}} = \overline{\lambda \vv{v}}$:
\begin{gather*}
    A\overline{\vv{v}} = \overline{A\vv{v}} = \overline{\lambda} \,\overline{\vv{v}}\\
    \overline{\vv{v}^T}A = \overline{\vv{v^T}}A^T = (A\overline{\vv{v}})^T = \overline{\lambda}\,\overline{\vv{v}^T}\\
    \lambda(\overline{\vv{v}^T}\vv{v}) = \overline{\vv{v}^T}A^T \hdots = \overline{\lambda}(\overline{\vv{v}^T}\vv{v})
\end{gather*}
So $(\lambda - \overline{\lambda})(\overline{\vv{v}^T}\vv{v}) = 0$ Because $\vv{v} \neq 0$, we can say that $\lambda = \overline{\lambda} $ which only happens if $\lambda$ is real.
\end{proof}
If $A$ is symmetric, then any eigenvalues that belong to different eigenvalues are orthogonal.
\begin{proof}
Using the fact that $A^T = A$ and $\vv{x} \vdot \vv{y} = \vv{x}^T\vv{y}$, we can quite easily show that $(\lambda_1 - \lambda_2)(\vv{x} \vdot \vv{y}) = 0$, since the eigenvalues are distinct, the dot product is zero and they are orthogonal.
\end{proof}
So now we can state the \textbf{Spectral Theorem}, basically that if $A$ is real and $n \times n$, then $A$ is symmetric if and only if it is orthogonally diagonalizable.
\begin{proof}
We already proved the forward direction of this theorem, so we just have to do the reverse. You can prove the reverse direction by induction.
\end{proof}
We can use the Spectral Theorem now:
\begin{gather*}
    ...
    \end{bmatrix}
    \begin{bmatrix}
    \vv{q_1}^T \\ \vdots \\ \vv{q_n}^T
    \end{bmatrix}\\
    = \begin{bmatrix}
        \lambda_1\vv{q_1} \hdots \lambda\vv{q_n}
    \end{bmatrix}
    \begin{bmatrix}
    \vv{q_1}^T \\ \vdots \\ \vv{q_n}^T
    \end{bmatrix}\\
    = \lambda_1\vv{q_1}\vv{q_1}^T + \lambda_2\vv{q_2}\vv{q_2}^T + \hdots + \lambda_n\vv{q_n}\vv{q_n}^T
\end{gather*}
This is called the \textbf{spectral decomposition} of $A$. Each of the things you're adding is a rank 1 matrix, and because $\vv{x} \vdot \vv{y} = \vv{x}^T\vv{y}$, $\vv{q_i}\vv{q_i}^T$ is the projection matrix for the subspace spanned by $\vv{q_i}$.
\end{document}
